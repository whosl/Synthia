...
export class LoopExecutor {
      ...
      switch (stage) {
        ...
        case "synthesize": {
          ...
          if (synth.status !== "succeeded") {
            return this.finish("fail_closed", `synthesize ended in non-success state ${synth.status}${synth.errorCode ? ` (${synth.errorCode})` : ""}`, this.toolArtifacts());
          }
          break;
        }
        case "implement": {
          ...
          if (impl.status !== "succeeded") {
            return this.finish("fail_closed", `implement ended in non-success state ${impl.status}${impl.errorCode ? ` (${impl.errorCode})` : ""}`, this.toolArtifacts());
          }
          break;
        }
      }
  ...
  private async resumeFromGate(): Promise<LoopResult> {
    ...
    if (state === "rejected" || state === "withdrawn") {
      ...
      return this.finish("fail_closed", `gate ${gate} was ${state} — stopping (fail-closed)`, this.allArtifacts());
    }
  ...
  private async registerDocArtifact(stage: StageId, doc: DocGeneration, artifactType: ArtifactType): Promise<void> {
    ...
    const rev = await this.deps.governance.registerCandidateArtifact({
      artifactId,
      artifactType,
