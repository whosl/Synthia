...
import {
  assertJumpLocalResidueObservation07ArtifactFrozen,
  ...
  runJumpLocalResidueObservation07,
} from "./scripts/invoke-m4f-jump-local-residue-observation-07.ts";
import {
  JUMP_LOCAL_RESIDUE_ARTIFACT as JUMP_LOCAL_RESIDUE_ARTIFACT_06,
  JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH as JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH_06,
  JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256 as JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256_06,
} from "./scripts/invoke-m4f-jump-local-residue-observation-06.ts";
import { runJumpLocalResidueParseGate06 } from
  "./scripts/invoke-m4f-jump-local-residue-parse-gate-06.ts";
import {
...
const TOKEN = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_OBSERVATION_20260827_07";
const ENV = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION_07";
const ENV_06 = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION_06";
const TOKEN_06 = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_OBSERVATION_20260827_06";
const PARSE_ENV_06 = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_PARSE_GATE_CONFIRMATION";
const PARSE_TOKEN_06 = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_PARSE_GATE_20260827_06";
...
afterEach(() => {
  ...
  delete process.env[ENV_06];
  delete process.env[PARSE_ENV_06];
});
...
describe("M4-F jump-local residue/liveness observation", () => {
  test("is confirmation-first, single-shot, and retains raw phase evidence on semantic failure", () => {
    ...
    expect(capture(() => runJumpLocalResidueObservation07({ invokePhase })).code)
    ...
    let seenName = "";
    let seenArtifact = "";
    const result = runJumpLocalResidueObservation07({
  ...
  test("freezes two bounded CIM snapshots and contains no active or sensitive capability", () => {
    expect(JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH).toBe(17_481);
    ...
    expect(JUMP_LOCAL_RESIDUE_ARTIFACT).toBe(JUMP_LOCAL_RESIDUE_ARTIFACT_06);
    expect(Buffer.from(JUMP_LOCAL_RESIDUE_ARTIFACT, "utf8")
      .equals(Buffer.from(JUMP_LOCAL_RESIDUE_ARTIFACT_06, "utf8"))).toBe(true);
    expect(JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH).toBe(JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH_06);
    expect(JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256).toBe(JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256_06);
    expect(() => assertJumpLocalResidueObservation07ArtifactFrozen()).not.toThrow();
    expect(() => assertJumpLocalResidueObservation07ArtifactFrozen(
      JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH + 1,
      JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256,
    )).toThrow("M4F_JUMP_LOCAL_RESIDUE_ARTIFACT_DRIFT");
    const source = readFileSync(
      new URL("./scripts/invoke-m4f-jump-local-residue-observation-07.ts", import.meta.url),
      "utf8",
    );
    const runStart = source.indexOf("export function runJumpLocalResidueObservation07");
    expect(source.indexOf("assertJumpLocalResidueObservation07ArtifactFrozen();", runStart))
      .toBeLessThan(source.indexOf("SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION_07", runStart));
  ...
  test("requires receiver CRLF framing and rejects LF-only raw stdout", () => {
    withConfirmation(() => {
      const accepted = phase(payload());
      const raw = Buffer.from(accepted.process.stdout_base64, "base64");
      expect(raw.toString("utf8")).toBe(
        `${JSON.stringify(accepted.identity)}\r\n${JSON.stringify(accepted.payload)}\r\n`,
      );
      expect(runJumpLocalResidueObservation07({ invokePhase: () => accepted }).status)
        .toBe("observed");

      const lfOnly = phase(payload());
      lfOnly.process.stdout_base64 = Buffer.from(
        `${JSON.stringify(lfOnly.identity)}\n${JSON.stringify(lfOnly.payload)}\n`,
        "utf8",
      ).toString("base64");
      expect(capture(() => runJumpLocalResidueObservation07({
        invokePhase: () => lfOnly,
      })).code).toBe("M4F_JUMP_LOCAL_RESIDUE_OUTPUT_INVALID");
    });
  });

  test("keeps _07 confirmation isolated from _06 observation and parse tokens", () => {
    let calls = 0;
    for (const [name, token] of [
      [ENV_06, TOKEN_06],
      [PARSE_ENV_06, PARSE_TOKEN_06],
    ] as const) {
      process.env[name] = token;
      expect(capture(() => runJumpLocalResidueObservation07({
        invokePhase: () => { calls += 1; return phase(payload()); },
    ...
    expect(capture(() => runJumpLocalResidueParseGate06({
      invokePhase: () => { parseCalls += 1; return phase(payload()); },
    })).code).toBe("M4F_JUMP_LOCAL_RESIDUE_PARSE_CONFIRMATION_REQUIRED");
    expect(parseCalls).toBe(0);
  });

  test("removes and audits the PowerShell 5.1 multiline command-style parse risk", () => {
    const rejected05Shape = [
      "if ($path -or",
      "  -not (Test-ReceiverCommandLine",
      "    $command",
      "    $expected)) {",
    ].join("\n");
    expect(findPowerShell51MultilineCommandInvocationRisks(rejected05Shape)).toEqual([2]);
    expect(findPowerShell51MultilineCommandInvocationRisks("$ok = Test-Thing\n  $value"))
      .toEqual([1]);
    expect(findPowerShell51MultilineCommandInvocationRisks("return Test-Thing\n  $value"))
      .toEqual([1]);
    expect(findPowerShell51MultilineCommandInvocationRisks(JUMP_LOCAL_RESIDUE_ARTIFACT)).toEqual([]);
    ...
    expect(JUMP_LOCAL_RESIDUE_ARTIFACT).not.toContain("-not (Test-ReceiverCommandLine");
  });
  ...
  test("keeps producer and validator parent correlation aligned across snapshots and 100ns ticks", () => {
    withConfirmation(() => {
      ...
      for (const candidate of cases) {
        expect(runJumpLocalResidueObservation07({ invokePhase: () => phase(candidate) }).status)
          .toBe("observed");
      }
    });
  });
  ...
  test("accepts composite PID reuse bindings and rejects fabricated exact duplicates", () => {
    ...
    withConfirmation(() => {
      expect(runJumpLocalResidueObservation07({
      ...
      for (const duplicate of [
        ...
        expect(capture(() => runJumpLocalResidueObservation07({ invokePhase: () => phase(duplicate) })).code)
  ...
  test("rejects state, disclaimers, count, ordering, PID, self, command, path, time, and schema drift", () => {
    ...
    withConfirmation(() => {
      for (const mutate of mutations) {
        const value = payload();
        mutate(value);
        expect(capture(() => runJumpLocalResidueObservation07({ invokePhase: () => phase(value) })).code)
          .toBe("M4F_JUMP_LOCAL_RESIDUE_OUTPUT_INVALID");
      }
    });
  });
  ...
  test("rejects phase/process/master evidence drift", () => {
    withConfirmation(() => {
      for (const mutate of [
        ...
        mutate(result);
        expect(capture(() => runJumpLocalResidueObservation07({ invokePhase: () => result })).code)
          .toBe("M4F_JUMP_LOCAL_RESIDUE_OUTPUT_INVALID");
      }
    });
  });
...
function phase(value: Record<string, unknown>): BoundJumpPhaseResult {
  const result: BoundJumpPhaseResult = {
    ...
    phase: "jump-local-residue-07:observe",
    identity: {
