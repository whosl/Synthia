...
import {
  CANDIDATE29_EVIDENCE_DIRECTORY,
  CANDIDATE29_ID,
  ...
  Candidate29Failure,
  buildCandidate29Payload,
  candidate29HasForbiddenMutation,
  candidate29Confirmation,
  executeCandidate29,
  planCandidate29,
  type Candidate29Config,
  type Candidate29Dependencies,
} from "./scripts/m4f-direct-attempt2-residue-inventory-29.ts";
...
const SOURCE = new URL("./scripts/m4f-direct-attempt2-residue-inventory-29.ts", import.meta.url).pathname;
const TEST_SOURCE = new URL("./m4f-direct-attempt2-residue-inventory-29.test.ts", import.meta.url).pathname;
...
function fixture() {
  ...
  let created = false;
  const dependencies: Candidate29Dependencies = {
    read(path) {
    ...
    createEvidence(path) {
      if (created || path !== CANDIDATE29_EVIDENCE_DIRECTORY) throw new Error("create");
      created = true;
    },
...
function failure(run: () => unknown): Candidate29Failure {
  try { run(); } catch (error) {
    expect(error).toBeInstanceOf(Candidate29Failure);
    return error as Candidate29Failure;
  }
  throw new Error("expected failure");
}

describe("M4-F Candidate 29 read-only inventory", () => {
  test("binds the parse-zero exact script and permits only read-only observation", () => {
    ...
    expect(candidate29HasForbiddenMutation(
      "$e=[ordered]@{remote_file_write_performed=$false;file_mutation_performed=$false;vivado_action_performed=$false}",
    )).toBe(false);
    expect(candidate29HasForbiddenMutation("$x=1;Out-File result.txt")).toBe(true);
    expect(candidate29HasForbiddenMutation("$x=1;Vivado.exe -mode batch")).toBe(true);
    expect(candidate29HasForbiddenMutation(value.payload.targetScript)).toBe(false);
  });

  test("plans without network and freezes empty stdin plus one attempt", () => {
    const value = fixture();
    expect(planCandidate29(value.config, value.dependencies)).toMatchObject({
  ...
  test("runs one effective audit and one empty-stdin read-only inventory", () => {
    const value = fixture();
    const exact = candidate29Confirmation(value.config, value.dependencies);
    expect(executeCandidate29(value.config, exact, value.dependencies)).toMatchObject({
      status: "observed",
    ...
    expect(value.writes.has(CANDIDATE29_EVIDENCE_DIRECTORY + "/inventory-record.json")).toBe(true);
  });

  test("wrong confirmation fails before any spawn", () => {
    const value = fixture();
    const exact = candidate29Confirmation(value.config, value.dependencies);
    expect(failure(() => executeCandidate29(value.config, exact + "x", value.dependencies)).detail.code)
      .toBe("M4F_CANDIDATE29_CONFIRMATION_REQUIRED");
    expect(value.calls).toHaveLength(0);
  });
  ...
  test("invalid output and remote failures are terminal and never retried", () => {
    ...
    const outputFailure = failure(() => executeCandidate29(
      invalid.config,
      candidate29Confirmation(invalid.config, invalid.dependencies),
      invalid.dependencies,
    ));
    expect(outputFailure.detail).toMatchObject({
      code: "M4F_CANDIDATE29_INVENTORY_OUTPUT_INVALID",
      retry_permitted: false,
    ...
    expect(failure(() => executeCandidate29(
      remote.config,
      candidate29Confirmation(remote.config, remote.dependencies),
      remote.dependencies,
    )).detail.code).toBe("M4F_CANDIDATE29_REMOTE_FAILED");
