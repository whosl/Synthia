...
function Get-Sid([object]$IdentityReference, [string]$Name) {
  try {
    ...
    Fail "M4F_JUMP_BOOTSTRAP_IDENTITY_UNRESOLVED" $Name
  }
}
...
function Assert-AdministrativePrincipal {
  ...
  if ($null -eq $identity -or $null -eq $identity.User) {
    Fail "M4F_JUMP_BOOTSTRAP_REQUIRES_ADMIN"
  }
...
function Assert-TrustedAncestorAcl([object]$Acl, [string]$Path) {
  ...
  if (-not $trustedSids.ContainsKey($ownerSid)) {
    Fail "M4F_JUMP_BOOTSTRAP_ANCESTOR_OWNER_UNTRUSTED" ("{0}|{1}" -f $Path, $ownerSid)
  }
  $replacementRights = @(
  ...
  foreach ($entry in $Acl.Access) {
    if ($entry.AccessControlType -eq $allow -and
      ...
      if (-not $trustedSids.ContainsKey($writerSid)) {
        Fail "M4F_JUMP_BOOTSTRAP_ANCESTOR_REPLACE_UNTRUSTED" ("{0}|{1}" -f $Path, $writerSid)
      }
    }
  }
}
...
function Assert-AncestorChain([string]$Path) {
  ...
  if (-not $fullPath.StartsWith($fullRoot + '\', [StringComparison]::OrdinalIgnoreCase)) {
    Fail "M4F_JUMP_BOOTSTRAP_ANCESTOR_SCOPE_INVALID" $Path
  }
  ...
  while ($null -ne $current) {
    if ($current -isnot [IO.DirectoryInfo] -or
      ...
      Fail "M4F_JUMP_BOOTSTRAP_ANCESTOR_INVALID" $current.FullName
    }
  ...
  Fail "M4F_JUMP_BOOTSTRAP_ANCESTOR_INCOMPLETE" $Path
}

function Get-VolumeFacts {
  $logicalDisks = @(Get-CimInstance Win32_LogicalDisk -Filter "DeviceID='C:'")
  if ($logicalDisks.Count -ne 1) { Fail "M4F_JUMP_BOOTSTRAP_LOGICAL_DISK_INVALID" }
  ...
  if ([int]$logicalDisk.DriveType -ne 3 -or
    ...
    Fail "M4F_JUMP_BOOTSTRAP_LOCAL_FIXED_NTFS_REQUIRED"
  }
  ...
  if ([string]$volume.FileSystem -cne "NTFS" -or
    ...
    Fail "M4F_JUMP_BOOTSTRAP_VOLUME_MAPPING_INVALID"
  }
...
function Assert-SameVolumeFacts([object]$Before, [object]$After) {
  foreach ($property in @(
    ...
    if ([string]$Before.$property -cne [string]$After.$property) {
      Fail "M4F_JUMP_BOOTSTRAP_VOLUME_MAPPING_DRIFT" $property
    }
  }
}

function New-BootstrapRootAcl {
...
function Assert-BootstrapRoot {
  $item = Get-Item -LiteralPath $bootstrapRoot -Force -ErrorAction Stop
  if ($item -isnot [IO.DirectoryInfo] -or
    (($item.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0)) {
    Fail "M4F_JUMP_BOOTSTRAP_ROOT_INVALID"
  }
  $streams = @(Get-Item -LiteralPath $bootstrapRoot -Stream * -ErrorAction Stop)
  if (@($streams | Where-Object { [string]$_.Stream -cne ':$DATA' }).Count -ne 0) {
    Fail "M4F_JUMP_BOOTSTRAP_ALTERNATE_STREAM"
  }
  if (@(Get-ChildItem -LiteralPath $bootstrapRoot -Force -ErrorAction Stop).Count -ne 0) {
    Fail "M4F_JUMP_BOOTSTRAP_NOT_EMPTY"
  }
  $acl = Get-Acl -LiteralPath $bootstrapRoot
  if ((Get-Sid $acl.Owner "bootstrap_owner") -cne $administratorsSid.Value -or
    -not $acl.AreAccessRulesProtected) {
    Fail "M4F_JUMP_BOOTSTRAP_BOUNDARY_INVALID"
  }
  $expectedRights = @{
  ...
  $rules = @($acl.Access)
  if ($rules.Count -ne $expectedRights.Count) { Fail "M4F_JUMP_BOOTSTRAP_ACE_COUNT_INVALID" }
  $seen = @{}
  foreach ($entry in $rules) {
    $sid = Get-Sid $entry.IdentityReference "bootstrap_access_rule"
    if (-not $expectedRights.ContainsKey($sid) -or
      ...
      Fail "M4F_JUMP_BOOTSTRAP_ACE_INVALID" $sid
    }
    $seen[$sid] = $true
  }
}
...
if (Test-Path -LiteralPath $bootstrapRoot) { Fail "M4F_JUMP_BOOTSTRAP_ALREADY_EXISTS" }
Assert-AncestorChain (Split-Path -Parent $bootstrapRoot)

# No cleanup or retry: any post-creation failure preserves evidence.
[void](New-Item -ItemType Directory -Path $bootstrapRoot -ErrorAction Stop)
Set-Acl -LiteralPath $bootstrapRoot -AclObject (New-BootstrapRootAcl)
Assert-BootstrapRoot
Assert-AncestorChain $bootstrapRoot
