...
export const CANDIDATE25_ID = "m4f-direct-attempt2-residue-parse-probe-prod-20260830-25";
...
export const CANDIDATE24_MANIFEST_SHA256 =
  "1427d18f070cf4990d1cbfc529fb82005bad544bd78ed7ee783c2088c00951e1";
export const CANDIDATE24_MANIFEST = {
  "confirmation.sha256": "ee56ce282e6f29877f6291d1cf1c97c4e01205fbda72c03607f67782662c5d95",
  "parser-loader.ps1": "f953f0e052da0326d2371e53a531198a32b05f881627da2c8a4c9bae7c48d492",
  "plan.canonical.json": "81c0e3a01494173781da756e5b8e0968321c33da7d9adff683edc13eeac8ab78",
  "probe-config.canonical.json": "5f490ca576751d7d39194a42d9e05d746c53e7a3dfb24a6fa59f92188824424e",
  "probe-record.json": "83064668adbd295aa64f3ac5cb6b8f709efbbee0730d38a6a68e0870185b7684",
  "remote-process.json": "e5d9ca1ad57b9c3fbff75fb2c53a5a2f7c4c9a6dc4570b789e8f3ddbd10cdc6f",
  ...
  "stdout.raw": "f8f521183bd1a722f14817cbcc526e2319ac65943df2e26ca17d62649347a2f6",
...
const TEST_PATH = fileURLToPath(new URL("../m4f-direct-attempt2-residue-parse-probe-25.test.ts", import.meta.url));
...
export interface Candidate25Config {
  schema: "synthia-m4f-direct-attempt2-residue-parse-probe-25-config.v1";
  probe_id: typeof CANDIDATE25_ID;
  evidence_directory: typeof CANDIDATE25_EVIDENCE_DIRECTORY;
  candidate24_evidence_directory: typeof CANDIDATE24_EVIDENCE_DIRECTORY;
  candidate24_evidence_manifest_sha256: typeof CANDIDATE24_MANIFEST_SHA256;
...
export interface Candidate25Dependencies {
...
export class Candidate25Failure extends Error {
  constructor(readonly detail: Record<string, unknown>) {
    super(String(detail.code ?? "M4F_CANDIDATE25_FAILED"));
  }
}
...
function fail(code: string, stage: string, extra: Record<string, unknown> = {}): never {
  throw new Candidate25Failure({
    schema: "synthia-m4f-direct-attempt2-residue-parse-probe-25-failure.v1",
    code,
    stage,
...
export function validateCandidate25Config(value: unknown): Candidate25Config {
  const config = object(value);
  const keys = [
    "candidate24_evidence_directory", "candidate24_evidence_manifest_sha256", "evidence_directory",
  ...
  if (!config || Object.keys(config).sort().join("|") !== keys.sort().join("|")
    || config.schema !== "synthia-m4f-direct-attempt2-residue-parse-probe-25-config.v1"
    || config.probe_id !== CANDIDATE25_ID
    || config.evidence_directory !== CANDIDATE25_EVIDENCE_DIRECTORY
    || config.candidate24_evidence_directory !== CANDIDATE24_EVIDENCE_DIRECTORY
    || config.candidate24_evidence_manifest_sha256 !== CANDIDATE24_MANIFEST_SHA256
    ...
    fail("M4F_CANDIDATE25_CONFIG_INVALID", "config");
  }
  return value as Candidate25Config;
}

function readExpected(
  dependencies: Candidate25Dependencies,
  path: string,
  expectedSha256: string,
): Buffer {
  let bytes: Buffer;
  try { bytes = dependencies.read(path); } catch {
    fail("M4F_CANDIDATE25_INPUT_MISSING", "local_preflight", { path });
  }
  if (sha256(bytes) !== expectedSha256) {
    fail("M4F_CANDIDATE25_INPUT_DRIFT", "local_preflight", { path });
  }
  return bytes;
}

export function buildCandidate25Payload(inventoryConfig: ResidueInventoryConfig) {
  ...
  const loader = [
    ...
    "Write-Output ('r25|{0}|{1}|{2}|{3}' -f ([Text.Encoding]::UTF8.GetByteCount($x)),$e.Count,$a.GetType().Name,($null -ne $a.EndBlock))",
  ].join(";");
  ...
  if (command.length > COMMAND_LIMIT || forbidden || parserCallCount !== 1) {
    fail("M4F_CANDIDATE25_PAYLOAD_INVALID", "config", {
...
function loadContext(rawConfig: unknown, dependencies: Candidate25Dependencies) {
  const config = validateCandidate25Config(rawConfig);
  ...
  const candidate24Names = Object.keys(CANDIDATE24_MANIFEST).sort();
  if (canonicalJson(dependencies.entries(CANDIDATE24_EVIDENCE_DIRECTORY))
    !== canonicalJson(candidate24Names)
    || sha256(canonicalJson(CANDIDATE24_MANIFEST) + "\n") !== CANDIDATE24_MANIFEST_SHA256) {
    fail("M4F_CANDIDATE25_CANDIDATE24_EVIDENCE_INVALID", "local_preflight");
  }
  for (const name of candidate24Names) readExpected(
    dependencies,
    CANDIDATE24_EVIDENCE_DIRECTORY + "/" + name,
    CANDIDATE24_MANIFEST[name as keyof typeof CANDIDATE24_MANIFEST],
  );
  ...
  if (transport.target.host !== TARGET_HOST
    ...
    fail("M4F_CANDIDATE25_TRANSPORT_INVALID", "local_preflight");
  }
  return { config, transport, payload: buildCandidate25Payload(inventoryConfig) };
}

function planFor(context: ReturnType<typeof loadContext>) {
  return {
    schema: "synthia-m4f-direct-attempt2-residue-parse-probe-25-plan.v1",
...
export function planCandidate25(rawConfig: unknown, dependencies: Candidate25Dependencies) {
  return planFor(loadContext(rawConfig, dependencies));
}

function confirmationFor(context: ReturnType<typeof loadContext>): string {
  return [
    "SYNTHIA_M4F_CANDIDATE25_EXACT_INVENTORY_PARSE_ONLY",
    CANDIDATE25_ID,
...
export function candidate25Confirmation(rawConfig: unknown, dependencies: Candidate25Dependencies): string {
...
function validateOutput(stdout: Buffer, context: ReturnType<typeof loadContext>) {
  ...
  try {
    ...
    fail("M4F_CANDIDATE25_STDOUT_INVALID", "network");
  }
  if (fields.length !== 5 || fields[0] !== "r25"
    || Number(fields[1]) !== context.payload.targetLength || fields[2] !== "0"
    || fields[3] !== "ScriptBlockAst" || fields[4] !== "True") {
    fail("M4F_CANDIDATE25_PARSE_REJECTED", "network", {
...
function write(dependencies: Candidate25Dependencies, directory: string, name: string, value: string | Buffer): void {
  try {
    dependencies.writeEvidence(directory + "/" + name, Buffer.isBuffer(value) ? value : Buffer.from(value));
  } catch {
    fail("M4F_CANDIDATE25_EVIDENCE_WRITE_FAILED", "evidence");
  }
}

export function executeCandidate25(
  rawConfig: unknown,
  confirmation: string,
  dependencies: Candidate25Dependencies,
) {
  const context = loadContext(rawConfig, dependencies);
  if (confirmation !== confirmationFor(context)) fail("M4F_CANDIDATE25_CONFIRMATION_REQUIRED", "local_preflight");
  ...
  try { dependencies.createEvidence(context.config.evidence_directory); } catch {
    fail("M4F_CANDIDATE25_EVIDENCE_CREATE_FAILED", "local_preflight");
  }
  ...
  if (effective.status !== 0 || effective.signal !== null || effective.errorCode !== null
    ...
    fail("M4F_CANDIDATE25_EFFECTIVE_FAILED", "local_preflight");
  }
  ...
  if (canonicalJson(pre) !== canonicalJson(initial)
    ...
    fail("M4F_CANDIDATE25_INPUT_DRIFT", "local_preflight");
  }
  ...
  if (canonicalJson(post) !== canonicalJson(initial)
    || confirmationFor(loadContext(rawConfig, dependencies)) !== confirmation) {
    fail("M4F_CANDIDATE25_POSTFLIGHT_DRIFT", "network_postflight");
  }
  if (remote.status !== 0 || remote.signal !== null || remote.errorCode !== null) {
    fail("M4F_CANDIDATE25_REMOTE_FAILED", "network", processEvidence(remote));
  }
  if (remote.stderr.length !== 0) fail("M4F_CANDIDATE25_STDERR_REJECTED", "network");
  const result = validateOutput(remote.stdout, context);
  const record = {
    schema: "synthia-m4f-direct-attempt2-residue-parse-probe-25-record.v1",
    probe_id: CANDIDATE25_ID,
...
const systemDependencies: Candidate25Dependencies = {
...
function main(): void {
  try {
    ...
    if (args.length === 3 && args[0] === "--plan") {
      const plan = planCandidate25(config, systemDependencies);
      process.stdout.write(JSON.stringify({ plan, confirmation: candidate25Confirmation(config, systemDependencies) }) + "\n");
      return;
    }
    if (args.length === 5 && args[0] === "--execute" && args[3] === "--confirmation") {
      process.stdout.write(JSON.stringify(executeCandidate25(config, args[4]!, systemDependencies)) + "\n");
      return;
    }
    throw new Error("usage");
  } catch (error) {
    const detail = error instanceof Candidate25Failure ? error.detail : {
      code: error instanceof Error ? error.message : "M4F_CANDIDATE25_UNEXPECTED",
      retry_permitted: false,
    };
