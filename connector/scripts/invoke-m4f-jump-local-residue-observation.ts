...
const CONFIRMATION = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_OBSERVATION_20260827_05";
  ...
  try {
    ...
    if ($current.fact.executable_path -cne $PowerShellPath -or
      -not (Test-ReceiverCommandLine
        $current.raw_command_line
        $PowerShellPath
        $ExpectedEncodedLength
        $ExpectedEncodedSha256
        $Ascii
        $Unicode)) {
...
export const JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256 = createHash("sha256")
  ...
  .digest("hex");
const EXPECTED_ARTIFACT_LENGTH = 17_467;
const EXPECTED_ARTIFACT_SHA256 = "7d35d79cb754c0690db3c0c2760897bfdf02f8ab4845dff8558d6d8a2930c1c2";
...
function validatePhaseEvidence(observation: BoundJumpPhaseResult): void {
  ...
  if (observation.schema !== "synthia-m4f-bound-phase.v1"
    || observation.phase !== "jump-local-residue:observe"
...
export function runJumpLocalResidueObservation(
  ...
  if (process.env.SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION !== CONFIRMATION) {
    failure("M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION_REQUIRED", "confirmation", null, null);
  }
  const dependencies = { ...defaultDependencies, ...overrides };
  let observation: BoundJumpPhaseResult;
  try {
    observation = dependencies.invokePhase("jump-local-residue:observe", JUMP_LOCAL_RESIDUE_ARTIFACT);
  } catch (error) {
