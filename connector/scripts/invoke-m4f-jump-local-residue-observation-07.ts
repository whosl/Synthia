...
const CONFIRMATION = "SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_OBSERVATION_20260827_07";
...
export function assertJumpLocalResidueObservation07ArtifactFrozen(
  length = JUMP_LOCAL_RESIDUE_ARTIFACT_LENGTH,
  hash = JUMP_LOCAL_RESIDUE_ARTIFACT_SHA256,
): void {
...
function validatePhaseEvidence(observation: BoundJumpPhaseResult): void {
  ...
  if (observation.schema !== "synthia-m4f-bound-phase.v1"
    || observation.phase !== "jump-local-residue-07:observe"
...
export interface JumpLocalResidue07Dependencies {
  invokePhase(phase: string, artifact: string): BoundJumpPhaseResult;
}

export interface JumpLocalResidue07Result {
  schema: "synthia-m4f-jump-local-residue-ceremony-07.v1";
  status: "observed";
  observation: BoundJumpPhaseResult;
}

const defaultDependencies: JumpLocalResidue07Dependencies = {
  invokePhase: (phase, artifact) => invokeBoundJumpPhase(phase, artifact),
};

export function runJumpLocalResidueObservation07(
  overrides: Partial<JumpLocalResidue07Dependencies> = {},
): JumpLocalResidue07Result {
  assertJumpLocalResidueObservation07ArtifactFrozen();
  if (process.env.SYNTHIA_M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION_07 !== CONFIRMATION) {
    failure("M4F_JUMP_LOCAL_RESIDUE_CONFIRMATION_REQUIRED", "confirmation", null, null);
  }
  const dependencies = { ...defaultDependencies, ...overrides };
  let observation: BoundJumpPhaseResult;
  try {
    observation = dependencies.invokePhase("jump-local-residue-07:observe", JUMP_LOCAL_RESIDUE_ARTIFACT);
  } catch (error) {
...
if (import.meta.main) {
  runCeremony(() => {
    process.stdout.write(`${JSON.stringify(runJumpLocalResidueObservation07())}\n`);
  });
}
