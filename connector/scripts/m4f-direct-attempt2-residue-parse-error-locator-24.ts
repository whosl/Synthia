...
const EXPECTED_PARSE_ERROR_COUNT = 18;

export const CANDIDATE24_ID = "m4f-direct-attempt2-residue-parse-error-locator-prod-20260830-24";
export const CANDIDATE24_EVIDENCE_DIRECTORY =
  "/private/tmp/m4f-direct-attempt2-residue-parse-error-locator-prod-20260830-24-evidence";
export const CANDIDATE23_FAILURE_EVIDENCE_DIRECTORY =
  "/private/tmp/m4f-direct-attempt2-residue-parse-probe-prod-20260830-23-evidence";
export const CANDIDATE23_FAILURE_MANIFEST_SHA256 =
  "833b1a523e290ae945db74d7220570f26ebaae7ef6658d2151659f6027e75576";
export const CANDIDATE23_FAILURE_MANIFEST = {
  "confirmation.sha256": "352303fb19d89d223eb3898cc2899434be43e5e8c2c4389ba64c87c42f3b47bb",
  "parser-loader.ps1": "69e3483a514aad32d79bfeebb2dfc026a947e285e79acff81f9ff8871e11e172",
  "plan.canonical.json": "db859fdeb9c2fdef3883ae414bbaf4db5be343bdc96269685d2d5cdad0d59e30",
  "probe-config.canonical.json": "6229eab3303786fea5d5f3b459f87ee0d83dea03e3f39a2f874f7c92a406b3ba",
  "remote-process.json": "73439a41a8c6447fed3884f05b1ab22c31ff15f0941d2423a03d443101d796dc",
  ...
  "stderr.raw": "4f2b6b3ed9201ca459db2dd042e0a137c4e58bfe8e15a068e45ad8535fa5b1ef",
  "stdout.raw": "a050d468acace34b642e28c1b386218d7e500fc846b590cd22b6433702f7c6d2",
...
const TEST_PATH = fileURLToPath(new URL("../m4f-direct-attempt2-residue-parse-error-locator-24.test.ts", import.meta.url));
...
export interface Candidate24Config {
  schema: "synthia-m4f-direct-attempt2-residue-parse-error-locator-24-config.v1";
  probe_id: typeof CANDIDATE24_ID;
  evidence_directory: typeof CANDIDATE24_EVIDENCE_DIRECTORY;
  candidate23_evidence_directory: typeof CANDIDATE23_FAILURE_EVIDENCE_DIRECTORY;
  candidate23_evidence_manifest_sha256: typeof CANDIDATE23_FAILURE_MANIFEST_SHA256;
...
export interface Candidate24Dependencies {
...
export class Candidate24Failure extends Error {
  constructor(readonly detail: Record<string, unknown>) {
    super(String(detail.code ?? "M4F_CANDIDATE24_FAILED"));
  }
}
...
function fail(code: string, stage: string, extra: Record<string, unknown> = {}): never {
  throw new Candidate24Failure({
    schema: "synthia-m4f-direct-attempt2-residue-parse-error-locator-24-failure.v1",
    code,
    stage,
...
export function validateCandidate24Config(value: unknown): Candidate24Config {
  const config = object(value);
  const keys = [
    "candidate23_evidence_directory", "candidate23_evidence_manifest_sha256", "evidence_directory",
  ...
  if (!config || Object.keys(config).sort().join("|") !== keys.sort().join("|")
    || config.schema !== "synthia-m4f-direct-attempt2-residue-parse-error-locator-24-config.v1"
    || config.probe_id !== CANDIDATE24_ID
    || config.evidence_directory !== CANDIDATE24_EVIDENCE_DIRECTORY
    || config.candidate23_evidence_directory !== CANDIDATE23_FAILURE_EVIDENCE_DIRECTORY
    || config.candidate23_evidence_manifest_sha256 !== CANDIDATE23_FAILURE_MANIFEST_SHA256
    ...
    fail("M4F_CANDIDATE24_CONFIG_INVALID", "config");
  }
  return value as Candidate24Config;
}

function readExpected(
  dependencies: Candidate24Dependencies,
  path: string,
  expectedSha256: string,
): Buffer {
  let bytes: Buffer;
  try { bytes = dependencies.read(path); } catch {
    fail("M4F_CANDIDATE24_INPUT_MISSING", "local_preflight", { path });
  }
  if (sha256(bytes) !== expectedSha256) {
    fail("M4F_CANDIDATE24_INPUT_DRIFT", "local_preflight", { path });
  }
  return bytes;
}

export function buildCandidate24Payload(inventoryConfig: ResidueInventoryConfig) {
  ...
  const loader = [
    ...
    "Write-Output ('r24|s|{0}|{1}' -f $e.Count,$a.GetType().Name)",
    "foreach ($v in $e){'r24|e|{0}|{1}|{2}' -f $v.ErrorId,$v.Extent.StartOffset,$v.Extent.EndOffset}",
    "'r24|c'",
  ].join(";");
  ...
  if (command.length > COMMAND_LIMIT || forbidden || parserCallCount !== 1) {
    fail("M4F_CANDIDATE24_PAYLOAD_INVALID", "config", {
...
function loadContext(rawConfig: unknown, dependencies: Candidate24Dependencies) {
  const config = validateCandidate24Config(rawConfig);
  ...
  const candidate23Names = Object.keys(CANDIDATE23_FAILURE_MANIFEST).sort();
  if (canonicalJson(dependencies.entries(CANDIDATE23_FAILURE_EVIDENCE_DIRECTORY))
    !== canonicalJson(candidate23Names)
    || sha256(canonicalJson(CANDIDATE23_FAILURE_MANIFEST) + "\n")
      !== CANDIDATE23_FAILURE_MANIFEST_SHA256) {
    fail("M4F_CANDIDATE24_CANDIDATE23_EVIDENCE_INVALID", "local_preflight");
  }
  for (const name of candidate23Names) readExpected(
    dependencies,
    CANDIDATE23_FAILURE_EVIDENCE_DIRECTORY + "/" + name,
    CANDIDATE23_FAILURE_MANIFEST[name as keyof typeof CANDIDATE23_FAILURE_MANIFEST],
  );
  ...
  if (transport.target.host !== TARGET_HOST
    ...
    fail("M4F_CANDIDATE24_TRANSPORT_INVALID", "local_preflight");
  }
  return { config, transport, payload: buildCandidate24Payload(inventoryConfig) };
}

function planFor(context: ReturnType<typeof loadContext>) {
  return {
    schema: "synthia-m4f-direct-attempt2-residue-parse-error-locator-24-plan.v1",
...
export function planCandidate24(rawConfig: unknown, dependencies: Candidate24Dependencies) {
  return planFor(loadContext(rawConfig, dependencies));
}

function confirmationFor(context: ReturnType<typeof loadContext>): string {
  return [
    "SYNTHIA_M4F_CANDIDATE24_EXACT_INVENTORY_PARSE_ONLY",
    CANDIDATE24_ID,
...
export function candidate24Confirmation(rawConfig: unknown, dependencies: Candidate24Dependencies): string {
...
function validateOutput(stdout: Buffer, context: ReturnType<typeof loadContext>) {
  ...
  try {
    ...
    fail("M4F_CANDIDATE24_STDOUT_INVALID", "network");
  }
  const summary = lines[0]?.split("|") ?? [];
  const complete = lines.at(-1);
  const errorCount = Number(summary[2]);
  const errorLines = lines.slice(1, -1);
  const errors = errorLines.map((line) => line.split("|")).map((fields) => ({
    error_id: fields[2] ?? "",
    start_offset: Number(fields[3]),
    end_offset: Number(fields[4]),
    fields,
  }));
  if (summary.length !== 4 || summary[0] !== "r24" || summary[1] !== "s"
    || errorCount !== EXPECTED_PARSE_ERROR_COUNT || summary[3] !== "ScriptBlockAst"
    || complete !== "r24|c" || errorLines.length !== errorCount
    || errors.some((error) => error.fields.length !== 5 || error.fields[0] !== "r24"
      || error.fields[1] !== "e" || !/^[A-Za-z][A-Za-z0-9]*$/u.test(error.error_id)
      || !Number.isSafeInteger(error.start_offset) || !Number.isSafeInteger(error.end_offset)
      || error.start_offset < 0 || error.end_offset < error.start_offset
      || error.end_offset > context.payload.targetLength)) {
    fail("M4F_CANDIDATE24_LOCATOR_REJECTED", "network", {
      parse_error_count: summary[2] ?? null,
      ast_type: summary[3] ?? null,
  ...
  return {
    ...
    ast_type: summary[3],
    errors: errors.map(({ fields: _fields, ...error }) => error),
    target_body_invoked: false,
    cim_executed: false,
    stdin_length: 0,
  };
...
function write(dependencies: Candidate24Dependencies, directory: string, name: string, value: string | Buffer): void {
  try {
    dependencies.writeEvidence(directory + "/" + name, Buffer.isBuffer(value) ? value : Buffer.from(value));
  } catch {
    fail("M4F_CANDIDATE24_EVIDENCE_WRITE_FAILED", "evidence");
  }
}

export function executeCandidate24(
  rawConfig: unknown,
  confirmation: string,
  dependencies: Candidate24Dependencies,
) {
  const context = loadContext(rawConfig, dependencies);
  if (confirmation !== confirmationFor(context)) fail("M4F_CANDIDATE24_CONFIRMATION_REQUIRED", "local_preflight");
  ...
  try { dependencies.createEvidence(context.config.evidence_directory); } catch {
    fail("M4F_CANDIDATE24_EVIDENCE_CREATE_FAILED", "local_preflight");
  }
  ...
  if (effective.status !== 0 || effective.signal !== null || effective.errorCode !== null
    ...
    fail("M4F_CANDIDATE24_EFFECTIVE_FAILED", "local_preflight");
  }
  ...
  if (canonicalJson(pre) !== canonicalJson(initial)
    ...
    fail("M4F_CANDIDATE24_INPUT_DRIFT", "local_preflight");
  }
  ...
  if (canonicalJson(post) !== canonicalJson(initial)
    || confirmationFor(loadContext(rawConfig, dependencies)) !== confirmation) {
    fail("M4F_CANDIDATE24_POSTFLIGHT_DRIFT", "network_postflight");
  }
  if (remote.status !== 0 || remote.signal !== null || remote.errorCode !== null) {
    fail("M4F_CANDIDATE24_REMOTE_FAILED", "network", processEvidence(remote));
  }
  if (remote.stderr.length !== 0) fail("M4F_CANDIDATE24_STDERR_REJECTED", "network");
  const result = validateOutput(remote.stdout, context);
  const record = {
    schema: "synthia-m4f-direct-attempt2-residue-parse-error-locator-24-record.v1",
    probe_id: CANDIDATE24_ID,
...
const systemDependencies: Candidate24Dependencies = {
...
function main(): void {
  try {
    ...
    if (args.length === 3 && args[0] === "--plan") {
      const plan = planCandidate24(config, systemDependencies);
      process.stdout.write(JSON.stringify({ plan, confirmation: candidate24Confirmation(config, systemDependencies) }) + "\n");
      return;
    }
    if (args.length === 5 && args[0] === "--execute" && args[3] === "--confirmation") {
      process.stdout.write(JSON.stringify(executeCandidate24(config, args[4]!, systemDependencies)) + "\n");
      return;
    }
    throw new Error("usage");
  } catch (error) {
    const detail = error instanceof Candidate24Failure ? error.detail : {
      code: error instanceof Error ? error.message : "M4F_CANDIDATE24_UNEXPECTED",
      retry_permitted: false,
    };
