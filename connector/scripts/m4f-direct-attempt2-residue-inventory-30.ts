...
export const CANDIDATE30_ID = "m4f-direct-attempt2-residue-inventory-prod-20260830-30";
...
const TEST_PATH = fileURLToPath(new URL("../m4f-direct-attempt2-residue-inventory-30.test.ts", import.meta.url));
...
export interface Candidate30Config {
  schema: "synthia-m4f-direct-attempt2-residue-inventory-30-config.v1";
  inventory_id: typeof CANDIDATE30_ID;
  evidence_directory: typeof CANDIDATE30_EVIDENCE_DIRECTORY;
...
export interface Candidate30Dependencies {
...
export class Candidate30Failure extends Error {
  constructor(readonly detail: Record<string, unknown>) {
    super(String(detail.code ?? "M4F_CANDIDATE30_FAILED"));
  }
}
...
function fail(code: string, stage: string, extra: Record<string, unknown> = {}): never {
  ...
  throw new Candidate30Failure({
    schema: "synthia-m4f-direct-attempt2-residue-inventory-30-failure.v1",
    code,
    stage,
...
export function validateCandidate30Config(value: unknown): Candidate30Config {
  ...
  if (!config || Object.keys(config).sort().join("|") !== keys.sort().join("|")
    || config.schema !== "synthia-m4f-direct-attempt2-residue-inventory-30-config.v1"
    || config.inventory_id !== CANDIDATE30_ID
    || config.evidence_directory !== CANDIDATE30_EVIDENCE_DIRECTORY
    ...
    fail("M4F_CANDIDATE30_CONFIG_INVALID", "config");
  }
  return value as Candidate30Config;
}

function readExpected(
  dependencies: Candidate30Dependencies,
  path: string,
  expectedSha256: string,
): Buffer {
  let bytes: Buffer;
  try { bytes = dependencies.read(path); } catch {
    fail("M4F_CANDIDATE30_INPUT_MISSING", "local_preflight", { path });
  }
  if (sha256(bytes) !== expectedSha256) {
    fail("M4F_CANDIDATE30_INPUT_DRIFT", "local_preflight", { path });
  }
  return bytes;
}

export function buildCandidate30Payload(inventoryConfig: ResidueInventoryConfig) {
...
export function candidate30HasForbiddenMutation(script: string): boolean {
  return /(?:^|[;\s])Stop-Process(?:[\s;]|$)|\.Kill\s*\(|(?:^|[;\s])Set-Content(?:[\s;]|$)|(?:^|[;\s])Out-File(?:[\s;]|$)|(?:^|[;\s])Vivado(?:\.exe|\s|;|$)|(?:^|[;\s])program_hw(?:_devices)?(?:[\s;]|$)/iu
    .test(script);
}

function loadContext(rawConfig: unknown, dependencies: Candidate30Dependencies) {
  const config = validateCandidate30Config(rawConfig);
  ...
  if (canonicalJson(dependencies.entries(CANDIDATE27_EVIDENCE_DIRECTORY))
    ...
    fail("M4F_CANDIDATE30_CANDIDATE27_EVIDENCE_INVALID", "local_preflight");
  }
  ...
  if (!candidate27Record || candidate27Record.status !== "exact_helpers_runtime_smoked"
    ...
    fail("M4F_CANDIDATE30_CANDIDATE27_RECORD_INVALID", "local_preflight");
  }
  ...
  if (canonicalJson(dependencies.entries(CANDIDATE28_EVIDENCE_DIRECTORY))
    ...
    fail("M4F_CANDIDATE30_CANDIDATE28_EVIDENCE_INVALID", "local_preflight");
  }
  ...
  if (!candidate28Record || candidate28Record.status !== "parsed_not_invoked"
    ...
    fail("M4F_CANDIDATE30_CANDIDATE28_RECORD_INVALID", "local_preflight");
  }
  ...
  if (transport.target.host !== TARGET_HOST
    ...
    fail("M4F_CANDIDATE30_TRANSPORT_INVALID", "local_preflight");
  }
  return { config, transport, targetConfig: inventoryConfig, payload: buildCandidate30Payload(inventoryConfig) };
}

function planFor(context: ReturnType<typeof loadContext>) {
  return {
    schema: "synthia-m4f-direct-attempt2-residue-inventory-30-plan.v1",
...
export function planCandidate30(rawConfig: unknown, dependencies: Candidate30Dependencies) {
  return planFor(loadContext(rawConfig, dependencies));
}

function confirmationFor(context: ReturnType<typeof loadContext>): string {
  return [
    "SYNTHIA_M4F_CANDIDATE30_EXACT_READ_ONLY_INVENTORY",
    CANDIDATE30_ID,
...
export function candidate30Confirmation(rawConfig: unknown, dependencies: Candidate30Dependencies): string {
...
function write(dependencies: Candidate30Dependencies, directory: string, name: string, value: string | Buffer): void {
  try {
    dependencies.writeEvidence(directory + "/" + name, Buffer.isBuffer(value) ? value : Buffer.from(value));
  } catch {
    fail("M4F_CANDIDATE30_EVIDENCE_WRITE_FAILED", "evidence");
  }
}

export function executeCandidate30(
  rawConfig: unknown,
  confirmation: string,
  dependencies: Candidate30Dependencies,
) {
  const context = loadContext(rawConfig, dependencies);
  if (confirmation !== confirmationFor(context)) fail("M4F_CANDIDATE30_CONFIRMATION_REQUIRED", "local_preflight");
  ...
  try { dependencies.createEvidence(context.config.evidence_directory); } catch {
    fail("M4F_CANDIDATE30_EVIDENCE_CREATE_FAILED", "local_preflight");
  }
  ...
  if (effective.status !== 0 || effective.signal !== null || effective.errorCode !== null
    ...
    fail("M4F_CANDIDATE30_EFFECTIVE_FAILED", "local_preflight");
  }
  ...
  if (canonicalJson(pre) !== canonicalJson(initial)
    ...
    fail("M4F_CANDIDATE30_INPUT_DRIFT", "local_preflight");
  }
  ...
  if (canonicalJson(post) !== canonicalJson(initial)
    || confirmationFor(loadContext(rawConfig, dependencies)) !== confirmation) {
    fail("M4F_CANDIDATE30_POSTFLIGHT_DRIFT", "network_postflight");
  }
  if (remote.status !== 0 || remote.signal !== null || remote.errorCode !== null) {
    fail("M4F_CANDIDATE30_REMOTE_FAILED", "network", processEvidence(remote));
  }
  if (remote.stderr.length !== 0) fail("M4F_CANDIDATE30_STDERR_REJECTED", "network");
  let result: Record<string, unknown>;
  try {
    result = validateResidueInventoryOutput(remote.stdout, context.targetConfig);
  } catch {
    fail("M4F_CANDIDATE30_INVENTORY_OUTPUT_INVALID", "network");
  }
  const record = {
    schema: "synthia-m4f-direct-attempt2-residue-inventory-30-record.v1",
    inventory_id: CANDIDATE30_ID,
    status: "observed",
...
const systemDependencies: Candidate30Dependencies = {
...
function main(): void {
  try {
    ...
    if (args.length === 3 && args[0] === "--plan") {
      const plan = planCandidate30(config, systemDependencies);
      process.stdout.write(JSON.stringify({ plan, confirmation: candidate30Confirmation(config, systemDependencies) }) + "\n");
      return;
    }
    if (args.length === 5 && args[0] === "--execute" && args[3] === "--confirmation") {
      process.stdout.write(JSON.stringify(executeCandidate30(config, args[4]!, systemDependencies)) + "\n");
      return;
    }
    throw new Error("usage");
  } catch (error) {
    const detail = error instanceof Candidate30Failure ? error.detail : {
      code: error instanceof Error ? error.message : "M4F_CANDIDATE30_UNEXPECTED",
      retry_permitted: false,
    };
