...
export interface ImplementRequest extends VivadoRequestBase { readonly operation: "implement"; readonly sources: readonly SourceInput[]; readonly top: string; readonly part: string; readonly constraints?: readonly ConstraintInput[]; readonly generateTrialBitstream?: boolean }
...
const XDC_QUERY_COMMANDS = new Set(["get_cells", "get_clocks", "get_drc_checks", "get_nets", "get_pins", "get_ports"]);
...
async function implementationVerdict(outputDir: string, exitCode: number, text: string, requireBitstream = true): Promise<{ status: VivadoResultStatus; errorCode?: string }> {
  ...
  for (const name of IMPLEMENTATION_OUTPUTS.filter((name) => requireBitstream || name !== "synthia.bit")) {
