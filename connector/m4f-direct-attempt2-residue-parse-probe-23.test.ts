...
import {
  CANDIDATE23_EVIDENCE_DIRECTORY,
  CANDIDATE23_ID,
  CANDIDATE22_FAILURE_EVIDENCE_DIRECTORY,
  CANDIDATE22_FAILURE_MANIFEST,
  CANDIDATE22_FAILURE_MANIFEST_SHA256,
  Candidate23Failure,
  buildCandidate23Payload,
  candidate23Confirmation,
  executeCandidate23,
  planCandidate23,
  type Candidate23Config,
  type Candidate23Dependencies,
} from "./scripts/m4f-direct-attempt2-residue-parse-probe-23.ts";
...
const SOURCE = new URL("./scripts/m4f-direct-attempt2-residue-parse-probe-23.ts", import.meta.url).pathname;
const TEST_SOURCE = new URL("./m4f-direct-attempt2-residue-parse-probe-23.test.ts", import.meta.url).pathname;
...
const EFFECTIVE = CANDIDATE22_FAILURE_EVIDENCE_DIRECTORY + "/ssh-effective-stdout.raw";
...
function fixture() {
  ...
  for (const name of Object.keys(CANDIDATE22_FAILURE_MANIFEST)) {
    const path = CANDIDATE22_FAILURE_EVIDENCE_DIRECTORY + "/" + name;
  ...
  const payload = buildCandidate23Payload(inventoryConfig);
  const success = Buffer.from([
    "r23", payload.targetLength, 0, "ScriptBlockAst", "True",
  ].join("|") + "\r\n");
  const config: Candidate23Config = {
    schema: "synthia-m4f-direct-attempt2-residue-parse-probe-23-config.v1",
    probe_id: CANDIDATE23_ID,
    evidence_directory: CANDIDATE23_EVIDENCE_DIRECTORY,
    candidate22_evidence_directory: CANDIDATE22_FAILURE_EVIDENCE_DIRECTORY,
  ...
  let created = false;
  const dependencies: Candidate23Dependencies = {
    read(path) {
    ...
    entries(path) {
      if (path !== CANDIDATE22_FAILURE_EVIDENCE_DIRECTORY) throw new Error("directory");
      return Object.keys(CANDIDATE22_FAILURE_MANIFEST).sort();
    },
    ...
    createEvidence(path) {
      if (created || path !== CANDIDATE23_EVIDENCE_DIRECTORY) throw new Error("create");
      created = true;
    },
...
function failure(run: () => unknown): Candidate23Failure {
  try { run(); } catch (error) {
    expect(error).toBeInstanceOf(Candidate23Failure);
    return error as Candidate23Failure;
  }
  throw new Error("expected failure");
}

describe("M4-F Candidate 23 exact inventory parse probe", () => {
  ...
  test("plans without network and freezes empty stdin plus one attempt", () => {
    const value = fixture();
    expect(planCandidate23(value.config, value.dependencies)).toMatchObject({
  ...
  test("runs one effective audit and one empty-stdin parse probe", () => {
    const value = fixture();
    const exact = candidate23Confirmation(value.config, value.dependencies);
    expect(executeCandidate23(value.config, exact, value.dependencies)).toMatchObject({
    ...
    expect(value.writes.has(CANDIDATE23_EVIDENCE_DIRECTORY + "/probe-record.json")).toBe(true);
  });

  test("wrong confirmation fails before any spawn", () => {
    const value = fixture();
    const exact = candidate23Confirmation(value.config, value.dependencies);
    expect(failure(() => executeCandidate23(value.config, exact + "x", value.dependencies)).detail.code)
      .toBe("M4F_CANDIDATE23_CONFIRMATION_REQUIRED");
    expect(value.calls).toHaveLength(0);
  });
  ...
  test("parse errors and remote failures are terminal and never retried", () => {
    ...
    parseError.setRemote(processResult([
      "r23", parseError.payload.targetLength, 1, "ScriptBlockAst", "True",
    ].join("|") + "\r\n"));
    const parseFailure = failure(() => executeCandidate23(
      parseError.config,
      candidate23Confirmation(parseError.config, parseError.dependencies),
      parseError.dependencies,
    ));
    expect(parseFailure.detail).toMatchObject({
      code: "M4F_CANDIDATE23_PARSE_REJECTED",
      retry_permitted: false,
      target_body_invoked: false,
      cim_executed: false,
    });
    ...
    expect(failure(() => executeCandidate23(
      remote.config,
      candidate23Confirmation(remote.config, remote.dependencies),
      remote.dependencies,
    )).detail.code).toBe("M4F_CANDIDATE23_REMOTE_FAILED");
