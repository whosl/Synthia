...
export class WorkerRuntime {
  ...
  private registration?: ConnectorRegistration; private discovery?: DiscoverySnapshot; private active = 0;
  ...
  private async route(path: string, e: RemoteEnvelope<unknown>): Promise<{ status: number; body: RemoteEnvelope<unknown> }> {
    if (!this.endpoint.project_scope.includes(e.project_id)) throw new Error("PROJECT_NOT_ALLOWED"); if (!this.endpoint.data_classification_scope.includes(e.classification)) throw new Error("CLASSIFICATION_NOT_ALLOWED");
    const p = e.payload as Record<string, unknown>;
    if (path === "/registration") { this.registration = { ...copy(this.endpoint), registration_state: "approved" }; return { status: 200, body: this.envelope(e, this.registration) }; }
    if (path === "/discover") { this.discovery = await this.execution.discover(); return { status: 200, body: this.envelope(e, this.discovery) }; }
    if (path === "/heartbeat") { if (!this.registration) throw new Error("NOT_REGISTERED"); if (!this.discovery) this.discovery = await this.execution.discover(); const now = this.clock(); this.registration = { ...this.registration, registration_state: "ready", discovered: copy(this.discovery), last_heartbeat_at: now.toISOString(), lease_expires_at: new Date(now.getTime() + this.endpoint.lease_seconds * 1000).toISOString(), capability_drift: this.discovery.connector_protocol_version !== this.endpoint.protocol_version || this.discovery.toolchain_profile_hash !== this.endpoint.toolchain_profile_hash }; return { status: 200, body: this.envelope(e, this.registration) }; }
    if (path === "/jobs/submit") { return this.submit(e, p.request as JobRequest, p.approval as Record<string, unknown> | undefined); }
  ...
  private async run(job: Job): Promise<void> { const workspace = join(this.root, job.id); await mkdir(workspace, { recursive: true }); await writeFile(join(workspace, "input"), job.request.input, "utf8"); job.state = "preparing"; job.state = "running"; try { const result = await this.execution.execute(copy(job.request), workspace); if (this.jobs.get(job.id)?.state === "cancelled") return; job.state = result.outcome === "success" ? "succeeded" : result.outcome === "timeout" ? "timeout" : result.outcome === "lost" ? "lost" : result.outcome === "unknown_effect" ? "unknown_effect" : "failed"; if (result.output !== undefined) { job.outputSha256 = sha256(result.output); job.evidence = result.evidence ?? { jobId: job.id, entries: [{ name: "output.txt", sha256: job.outputSha256, sizeBytes: new TextEncoder().encode(result.output).byteLength, mediaType: "text/plain" }] }; } } catch { job.state = "failed"; } }
