...
import { Archive, ChevronRight, FolderKanban, Pencil, Plus, RefreshCw, Search, Trash2 } from 'lucide-react'
...
const DEFAULT_FORM: CreateProjectPayload = {
  name: '',
  root_path: '',
  manifest_path: '',
  xpr_path: '',
...
function projectFormFrom(p: Project): CreateProjectPayload {
  return {
    name: p.name,
    ...
    xpr_path: p.xpr_path || '',
    part: p.part || '',
    board_part: p.board_part || undefined,
    top_module: p.top_module || undefined,
  }
}
...
export default function ProjectsPage() {
  ...
  const submitProject = () => {
    ...
    const payload = {
      ...
      board_part: form.board_part,
      top_module: form.top_module,
    }
    if (editing) {
  ...
  const formPanel = (showCreate || editing) && (
    <Panel title={editing ? `Edit project — ${editing.name}` : 'Create project'}>
      <div className="project-create-form">
        ...
        <label>
          <span>Part</span>
          <input className="input mono" value={form.part || ''} onChange={(e) => setForm({ ...form, part: e.target.value })} />
        </label>
