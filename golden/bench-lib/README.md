...
2. **常量密集标准的规范数据附录（T1 DES 发现的硬规则）**：标准自身定义的高熵常量表（置换表/S 盒/轮常量/指令编码/寄存器复位值等）**必须随需求输入包提供权威原文**（`doc/std/<标准>-tables.md`，双实现交叉验证后入库）。红线区分：**结构信息（模块划分/状态机形态/流水线级数）禁止**，**规范数据（标准原文常量）必须附** — 模型凭记忆转录 ~500 个数字会出现单点错误，雪崩效应使 KAT mismatch 无法反推是哪个表错（p16 实证：物理指标同级、7 轮定位到常量表但盲测隔离下无法校正）。
3. **登记参考实现**：参考 RTL 入工作区并登记哈希（作为 p3 型"人工基线"），跑四步验证拿到 golden 指标（WNS/LUT/FF/通过场景）。
4. **盲测**：新项目只种入需求文档，用 p 系列协议（见下）。
5. **对比报告**：进 `specs/research/`，沿用 p5-p10 的度量口径。
