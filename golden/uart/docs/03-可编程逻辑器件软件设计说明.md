...
| 属性 | 内容 |
|---|---|
| 文档标识及版本 | UART-DOC-003 / v2.5 candidate |
...
| 版本 | 日期 | 修改内容 | 修改人 |
|---|---|---|---|
| v2.5 candidate | 2026-08-28 | 在官方 DIFF_SSTL15 约束上经 Mac→66 直连重跑并确认综合、布局布线、methodology/CDC/DRC/STA 与资源结果 | Agent 辅助拟制，待授权角色确认 |
| v2.4 candidate | 2026-08-27 | 实现 VC709 板级包装、MMCM/复位/CP2103 回显与候选 XDC，纳入真实 methodology/CDC/DRC/STA 结果 | Agent 辅助拟制，待授权角色确认 |
| v2.3 candidate | 2026-08-27 | 切换当前目标到 XC7VX690T，补充板级包装边界、115200 扩展回归与中等保证验证设计；K70T 数据保留为历史 exploratory 证据 | Agent 辅助拟制，待授权角色确认 |
| v2.2 candidate | 2026-08-26 | 同步完整标准参考清单、来源边界和上游 v2.2；设计与 RTL 不变 | Agent 辅助拟制，待授权角色确认 |
| v2.1 candidate | 2026-08-26 | 同步 GJB 438B-2009 参考集和上游候选文档版本；设计与 RTL 不变 | Agent 辅助拟制，待授权角色确认 |
...
设计对象为 GOLDEN-UART 项目的 UART 收发器 PLDS。通用核顶层为 `uart_top`，板级包装为 `uart_board_top`；目标为 VC709 / `xc7vx690tffg1761-2`，已由真实 Connector/Vivado 2021.1 通过 Mac→66 直连查询并完成修订后 `DIFF_SSTL15` 预码流实现。设计版本 v2.5 candidate，无发布号或批准基线。

### 1.2 系统概述

设计在 100 MHz 单时钟域中实现参数化 8N1 全双工 UART，9600 为基线，115200 为扩展回归。通用核包含顶层装配、发送状态机及位使能、接收同步与中点采样逻辑；板级包装只负责权威板卡时钟/复位和板载 USB-UART 适配。串行输入是唯一已识别的异步输入。
...
| UART-DOC-001 | PLDS 研制任务书 v2.5 | candidate |
| UART-DOC-002 | PLDS 需求规格说明 v2.5 | candidate |
...
flowchart TB
  ...
  WRAP[uart_board_top / 200→100 MHz / reset sync] <-->|clk/rst/txd/rxd| TOP
  BOARD[VC709 CP2103 USB-UART / 115200 echo] <--> WRAP
```

TX 和 RX 共享 `clk/rst`，数据通路独立。通用核全部为 Verilog-2001 可综合 RTL；`baud_tick` 仅是同拍时钟使能。`uart_board_top` 例化 IBUFDS、MMCME2_BASE 和 BUFG，将 VC709 的 200 MHz 差分时钟生成 100 MHz，按 MMCM LOCKED 进行无 LUT 毛刺的异步断言/两拍同步释放，并把收到的 115200 字节回显到 CP2103。
...
| UART-DES-MOD-005 | `uart_board_top` / `rtl/uart_board_top.v` | VC709 时钟、复位和板载 USB-UART 回显 | 按 UG887 v1.6 实现；真实 690T 预码流验证通过 |
...
| `BAUD_RATE` | 9,600（基线）/ 115,200（扩展） | bit/s |
| `CLKS_PER_BIT` | 10,416 / 868 | 整数向下取整 |
| 位时间 | 104.16 µs / 8.68 µs | 实际约 9600.6144 / 115207.37 bit/s |
...
本设计无地址映射或存储空间。通用核无厂商 IP、BRAM/DSP；板级包装显式使用 IBUFDS、MMCME2_BASE 和 BUFG。VC709 候选管脚与电气约束已依据 UG887 v1.6 实现并用入库 XTP213/Master XDC 交叉复核，异步 UART 不设置虚构的同步 I/O delay；实物板卡修订和系统 I/O 预算仍未闭合。仓库旧 K70T XDC/STA 只保留为历史 exploratory 输入。
...
| 验证对象 | 方法 | 证据/状态 |
|---|---|---|
| 功能、接口、异常、容差、吞吐 | XSim 行为仿真 + Icarus 交叉检查 | VC709/690T 9600 与 115200 均完成 10/10 exploratory 回归；旧外部 9 用例仅作历史交叉证据 |
| 端口和语法 | Vivado validate_sources | `xc7vx690tffg1761-2` 真实 Connector 校验成功 |
| 资源 | Vivado 综合/实现 | 55 LUT、62 FF、0 BRAM、0 DSP |
| 内部 100 MHz 时序 | Vivado route 后 STA | WNS 7.630 ns、WHS 0.093 ns、TNS/THS 0；异步 UART I/O 不作同步预算结论 |
| 候选 XDC 与 bitstream 边界 | route/DRC + `stopBeforeBitstream=true` | DRC 0 违规；Tcl 不含 `write_bitstream`，结构化结果为 `bitstreamGenerated=false` |
| CDC/RDC | `report_cdc` + 结构审查 | 仅 CDC-3 Info ×1，对应两级 `ASYNC_REG` RX 同步器；独立批准仍缺失 |
| 覆盖率、门级/SDF | 中等保证覆盖策划；门级/SDF 由验证/质量角色裁定 | 代码/功能覆盖分母、目标和排除规则未批准；未执行 |
| 板级确认 | PLDSVTP/VTD/VTR | XTP213/Master XDC 已入库复核；实物板卡修订确认、仪器、已批 bitstream 和规程缺失，仍阻塞 |
