...
| 属性 | 内容 |
|---|---|
| 文档标识及版本 | UART-DOC-003 / v2.3 candidate |
...
设计对象为 GOLDEN-UART 项目的 UART 收发器 PLDS。通用核顶层为 `uart_top`，规划的板级包装为 `uart_board_top`；目标器件为 XC7VX690T，候选 part `xc7vx690tffg1761-2` 尚未由真实 Connector/Vivado 确认。设计版本 v2.3 candidate，无发布号或批准基线。
...
| UART-DOC-002 | PLDS 需求规格说明 v2.3 | candidate |
...
flowchart TB
  ...
  WRAP[uart_board_top / blocked] <-->|clk/rst/txd/rxd| TOP
  BOARD[板载 USB-UART / 权威板级材料待提供] <--> WRAP
```

TX 和 RX 共享 `clk/rst`，数据通路独立。通用核全部为 Verilog-2001 可综合 RTL，无 IP 核、黑盒、BRAM、DSP 或派生时钟；`baud_tick` 仅是同拍时钟使能。`uart_board_top` 在板卡身份、原理图和 Master XDC 提供前只保留设计边界，不得猜测实例化或约束。
...
| UART-DES-MOD-005 | `uart_board_top` / 待建立 | 板级时钟、复位和板载 USB-UART 映射 | blocked；只能根据权威板级材料实现 |
...
本设计无地址映射、存储空间或 IP 核。精确板卡、正式管脚、I/O 时序与电气约束缺失；`uart_board_top` 和正式 690T XDC 因而阻塞。仓库现有 K70T XDC/STA 仅为历史 exploratory 输入，不得解释为当前目标的约束或时序证据。
...
| 验证对象 | 方法 | 证据/状态 |
|---|---|---|
| 功能、接口、异常、容差、吞吐 | XSim 行为仿真 + Icarus 交叉检查 | 9600 历史 10 个内部用例、9 个外部用例 exploratory 通过；115200 全量扩展回归待执行 |
| 端口和语法 | Vivado validate_sources | K70T 历史 exploratory 通过；690T 待精确 part 确认后重跑 |
| 资源 | Vivado 综合 | K70T 历史 exploratory：63 LUT/70 FF，BRAM/DSP=0；非 690T 证据 |
| 内部 100 MHz 时序 | Vivado 实现阶段 STA | K70T 历史 exploratory：Setup WNS 6.699 ns、Hold WHS 0.114 ns；690T 与正式 I/O 未闭合 |
| 正式 XDC fail-closed | 实现/DRC 负向验证 | NSTD-1/UCIO-1 阻止 bitstream |
| CDC/RDC | 结构审查 + 静态工具 | 两级同步结构已实现；中等保证要求独立审查，批准报告缺失 |
