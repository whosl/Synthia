...
| 待批准 | 设计负责人 |
| 修订日期 | 2026-08-26 |
| 上游 | UART-DOC-002《PLDS 需求规格说明》v1.2 candidate |
...
| `uart_rx` | leaf | `clk/rst` | 接收状态机：rxd 两级同步、同步后下降沿检测、半位偏移中点采样、帧错误丢弃与恢复 | `clk, rst, rxd` | `rx_data[7:0], rx_done, frame_err` |
...
flowchart TB
    subgraph TOP["uart_top（单时钟域 clk，100 MHz；rst 同步高有效）"]
        ...
        RX["uart_rx 接收通道\nIDLE/START/DATA/STOP\nrxd_meta/rxd_sync 两级同步 + rxd_sync_d 边沿历史 + clk_cnt 中点采样"]
    end
...
说明：发送通道的位时基准由 `baud_gen` 提供（仅发送期间计数，ce=tx_busy，帧起点自动对齐）；接收通道以 `rxd_sync_d/rxd_sync` 识别同步后下降沿，并由内部 `clk_cnt` 从该事件起计数，半位后确认起始位、随后每隔整位在中点采样。两通道节拍源相互独立，满足全双工独立工作要求（UART-SRS-FUN-008）。
...
| `rx_data` | output | 8 | data | 最近一个合法接收字节；仅在 rx_done 脉冲时更新，错误帧保持不变 |
| `rx_done` | output | 1 | status | 有效字节接收完成，单时钟周期脉冲（停止位中点采样为高） |
| `frame_err` | output | 1 | status | 错误帧被丢弃，单时钟周期脉冲（停止位中点采样为低）；与 rx_done 互斥，不锁存 |
...
| `rxd_sync` | `uart_rx` 第二级同步寄存器 | `uart_rx` 状态机、`rxd_sync_d` | 功能逻辑使用的同步后信号；复位值为 1 |
| `rxd_sync_d` | `uart_rx` 同步后历史寄存器 | `uart_rx` 状态机 | 保存 rxd_sync 前一拍值，与 rxd_sync 共同检测下降沿；复位值为 1，不属于 CDC 同步级 |
...
| CDC | 唯一异步输入 `rxd`，经 `rxd_meta`、`rxd_sync` 两级寄存器同步后使用（CDC-001），两级均标记 ASYNC_REG；`rxd_sync_d` 只保存同步后历史、不属于同步链；`tx_start/tx_data` 约定为同步输入 |
...
`rxd` → 两级寄存器同步（rxd_meta → rxd_sync）→ `rxd_sync_d` 保存前一拍 → IDLE 仅检测同步后下降沿（rxd_sync_d=1 且 rxd_sync=0）→ START 计数到 `(CLKS_PER_BIT−1)/2 = 5207` 后确认起始位 → DATA 每隔 10416 拍在中点采样 8 位入 data_reg（LSB 先收）→ STOP 中点校验：为高则更新 rx_data 并产生 rx_done，为低则保持 rx_data 并仅产生 frame_err → 回 IDLE。坏停止位持续为低时不会再次启动，线路回高后才可检测下一下降沿。
...
| UART-RISK-002 | rx_done/frame_err 为互斥的单拍脉冲，外部若采样不及时可能丢失指示 | 外部逻辑需在脉冲当拍分别捕获有效字节或错误帧 | 接口契约 §4.1 已注明脉冲语义；TB 已按当拍捕获验证 |
