...
| 待批准 | 设计负责人 |
| 修订日期 | 2026-08-26 |
| 上游 | UART-DOC-002/003 v1.1 candidate |
...
同 UART-DOC-001 §2；候选设计输入为 UART-DOC-002/003 v1.1。
...
采样定时：内部 clk_cnt 从检测到同步后低电平起计数；START 状态在 `clk_cnt==5207` 时确认起始位；DATA/STOP 状态在 `clk_cnt==10415` 时采样。
...
| `STOP` | 停止位校验 | clk_cnt 计数至 10415 | 中点采样：rx_data←data_reg，rx_done←1，frame_err←~rxd_sync，→ `IDLE` |
...
| `rxd_sync` | 1 | uart_rx | 1 | 异步输入同步第二级，功能逻辑唯一使用点 | 标记 ASYNC_REG |
...
| 异常 | 检测 | 锁存/报告 | 恢复 | 验证 |
|---|---|---|---|---|
| 帧错误（停止位为低） | STOP 中点采样 | frame_err 与 rx_done 同拍单拍；rx_data 仍更新 | 当拍回 IDLE；留出空闲后可收下一帧 | TB 坏停止位 + 正常恢复帧 |
