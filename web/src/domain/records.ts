 ...
 * 不应该往回塞技术字段。本模块反过来：从同一份 `TaskRunDetail` 里取
...
import type { TaskRunDetail } from "../api/types.ts";
...
/**
 * `TaskRunDetail` → 运行记录面板的 job 列表，按 `evidence` 原有顺序（即执行
 * 顺序）保留，逐条关联同 jobId 的 tool_call audit 事件取 ts/errorCode，并按
 * operation 计算「第几轮」。
 */
export function buildRecordJobs(detail: TaskRunDetail): RecordJob[] {
